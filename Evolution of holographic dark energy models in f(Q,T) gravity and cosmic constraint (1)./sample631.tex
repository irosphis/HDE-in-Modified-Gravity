...
The holographic dark energy (HDE) model is an approach to understanding the nature of dark energy based on the holographic principle. This principle suggests that the information content of a physical system in a given region of space can be described by a lower-dimensional theory on the boundary of that region. In cosmology, it has been applied to describe the energy density of dark energy in terms of the holographic principle, with the idea that the dark energy density is related to the largest length scale of the universe, typically chosen to be the future event horizon or the particle horizon (\cite{LI20041}). In the HDE model, the energy density of dark energy is typically expressed as
\begin{equation}
    \rho_\text{de} = 3c^2 M_p^2 L^{-2}
\end{equation}
where $L$ is the characteristic length scale of the universe, and $c$ is free parameter, $M_p$ denotes planck mass here we set it as 1. In order to understand the characteristic properties of the dark energy driving the recent acceleration of the Universe, we need to parameterize its EoS
\begin{equation}
    w_\text{de}=\frac{p_\text{de}}{\rho_\text{de}}
\end{equation}
Consider a simple case, the HDE energy density in Hubble cut-off can be described as 
\begin{equation}
