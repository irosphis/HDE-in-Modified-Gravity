...
Weyl in 1918 introduced an extension of Riemannian geometry, using a non-metricity tensor $Q_{\alpha \mu \nu}=\nabla_\alpha g_{\mu \nu}=-w_\alpha g_{\mu \nu}$, which describes how the length of a vector changes during parallel transport where $w_\alpha$ coincides with those of the electromagnetic potentials (\cite{Weyl:1918ib}). Cartan geometry generalizes Riemannian geometry by introducing torsion whose connection is asymmetric, with a torsion tensor $T^{\alpha }{}_{\mu \nu }$ that quantifies how vector twists as it is parallel transported.
...
\end{equation}
Where torsion tensor is defined as $T^{\alpha}{}_{\mu\nu}\equiv 2\Gamma^{\alpha}{}_{[\mu\nu]}$. The Last term is the distortion tensor $L^{\alpha}{}_{\mu\nu}$, given by
\begin{equation}
L^{\alpha}{}_{\mu\nu} = \frac{1}{2}Q^{\alpha}_{}{\mu\nu}-Q_{(\mu}{}^{\alpha }{}_{\nu)}
\end{equation}
...
\begin{equation}
    q = -1+\frac{3 (\alpha +1) \left(m-(2 \alpha +1) c^2 \text{wde}\right)}{(3 \alpha +2) m}
\end{equation}
...
\end{equation}
where $\dot{H}(t)=\frac{d}{dt}H(t)=-\frac{d H(z)}{dz}H(z)(1+z)$
